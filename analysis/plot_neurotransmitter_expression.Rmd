...
nt_receptors <- read_sheet(markers_by_group, sheet = "NT receptors")
histamine_synthesis <- read_sheet(markers_by_group, sheet = "Histamine synthesis")
...
tumors <- readRDS("/tgen_labs/banovich/BCTCSF/Heini/13384_Tumor/tumor_integrated_UPN109pre_noUPN208_soupX_snn_metadata_no4_7_11.rds")

tumors$lineage <- ifelse(tumors$celltype %in%  c("Olig1", "Olig2"), "Olig",
                         ifelse(tumors$celltype %in% c("Tumor1", "Tumor2", "Tumor3", "Tumor4", "Tumor5", "Tumor6"), "Tumor",
                                ifelse(tumors$celltype %in% c("Lymph1", "Lymph2"), "Lymph",
                                       ifelse(tumors$celltype == "Fibroblast", "Fibroblast", "Myel"))))
...
DotPlot(tumors,
        group.by = "celltype",
        features = unique(nt_receptors$Gene)) +
  theme_classic() +
  coord_flip() +
  RotatedAxis()
...
```{r, message = F, warning = F, fig.width = 18, fig.height = 64}
...
DotPlot(tumors,
        group.by = "celltype",
        features = unique(histamine_synthesis$Gene)) +
  theme_classic() +
  coord_flip() +
  RotatedAxis()
...
```{r, message = F, warning = F, fig.width = 18, fig.height = 12}
